...
const WorkOrderGroupDetailsModal = ({
    ...
    const {
        ...
        const tableRows = Object.values(accountMap).map((account) => {
            ...
            // Get the work order group's created_at date or work order's created_at date
            const accountCreatedAt =
                group?.created_at || account.latestStep?.workOrder?.created_at;
    ...
    return (
        ...
        >
            ...
            {/* Table Content */}
            <DialogBody
                className={`p-0 flex-1 flex flex-col min-h-0 ${showChecklistTable ? "overflow-hidden" : "overflow-y-auto"
                    }`}
            >
                {isLoading ? (
