...
const AddPriceVersionModal = ({ modalRef }) => {
    //States
    const { fetchPropertyNamesWithIds, propertyNamesList } = usePropertyNamesWithIds();
    ...
    return (
        <dialog className="modal w-[700px] rounded-[10px] bg-custom-grayFA backdrop:bg-black/50" ref={modalRef}>
            <div className=' px-[50px] mb-5 rounded-lg'>
                <div className=''>
                    <div method="dialog" className="pt-3 flex justify-end -mr-[40px]">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center h-[31px] w-[380px] border rounded-md text-sm overflow-hidden">
                        ...
                        <input
                            ...
                            type="text"
                            onChange={(event) => setSelectedTowerPhase(event.target.value)}
                    ...
                    <table className=''>
                        <thead>
                            <tr className='h-[83px] flex gap-[30px] items-center text-custom-grayA5 montserrat-semibold text-sm'>
                                <th className='rounded-tl-[10px] pl-[10px] w-[165px] text-left'>Version</th>
                        ...
                        <div className='shadow-custom5 rounded-[10px] overflow-hidden w-[600px]'>
                                ...
                                <tbody className='' key={index}>
                                    <tr className='h-[66px] text-sm border-separate bg-white'>
                                        <td className='px-[10px]'>
                                            <input type="text" name="name" className='pl-3  border border-custom-grayF1 rounded-[5px] h-[31px]'
                                                value={form.name}
                                                onChange={(event) => handlePriceVersionInputChange(event, index)}
                                            />
                                        </td>
                                        <td className='px-[10px]'>
                                            <input
                                        ...
                                        <td className='px-[10px]'>
                                            <input
                                                ...
                                                className='pl-3 w-[100px] border border-custom-grayF1 rounded-[5px] h-[31px]'
                                                onChange={(event) => handlePriceVersionInputChange(event, index)}
                                                value={form.no_of_allowed_buyers}
                                            />
                                        </td>
                                        <td className='px-[10px] mt-4 flex items-center gap-x-2'>
                                            <input className='pl-3 w-[90px] border border-custom-grayF1 rounded-[5px] h-[31px] outline-none' readOnly />
                                            <span>
                            ...
                            <div className='flex justify-center'>
                                <button className='h-[24px] w-[235px] my-[15px] flex gap-[10px] justify-center items-center text-sm' onClick={handleAddFields}>
                    ...
                    <div className='flex justify-center my-3'>
                        <button className='w-[129px] h-[37px] text-white montserrat-semibold text-sm gradient-btn2 rounded-[10px] hover:shadow-custom4'
