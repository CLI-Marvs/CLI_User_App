...
const SummaryRatingDetails = ({ surveyRatings }) => {
  ...
  // Pagination logic
  const totalPages = Math.ceil(filteredRatingDetails.length / itemsPerPage) || 1;
        ...
        {Object.entries(emojiMap)
          ...
          .map(([rating, imageUrl]) => (
            ...
            >
              ...
              <span className={`${selectedRating === Number(rating) ? "" : "text-[#9A9A9A]"}`}>
                ({ratingCounts[rating]})
              </span>
            </button>
          ))}
        ...
        <div className="p-4 flex justify-between items-center text-sm text-gray-700">
          ...
          <div className="flex gap-3 items-center">
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1 border-[.6px] rounded-[4px] font-medium ${
                currentPage === 1
