...
use App\Mail\CommentNotifEmail;
    ...
    public function handle(Mailer $mailer)
    {
        try {
            $mailer->to($this->employee_email)
                ->send(new CommentNotifEmail($this->employee_email, $this->assignee_name, $this->data));
            // Log::info('Emails sent successfully', ['emails' => $this->email]);
        } catch (\Exception $e) {
            Log::error('Error in handle function of CommentNotify', [
                'error' => $e->getMessage(),
