...
const Reports = () => {
    ...
    const TabItem = ({ isActive, onClick, icon, label }) => (
        <div
            onClick={onClick}
            className={`flex items-center gap-1 pb-1 cursor-pointer border-b-2 transition-all duration-300 ${isActive ? "border-custom-solidgreen" : "border-transparent"
                }`}
        >
            <div
                className={`rounded-full w-[30px] h-[30px] flex items-center justify-center transition-all duration-300 ${isActive ? "bg-custom-solidgreen" : "bg-[#BDBDBD]"
                    }`}
            >
                <img src={icon} alt="" className="text-white" />
            </div>
            <span
                className={`transition-all duration-300 ${isActive ? "text-custom-solidgreen" : "text-[#212121]"
                    }`}
            >
                {label}
            </span>
